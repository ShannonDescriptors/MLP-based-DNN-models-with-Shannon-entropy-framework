### This script could run an MLP only model for predicting BEI values of molecules (of IC50 of Tissue Factor Pathway Inhibitor) using combination of Morgan Fingerprint, Shannon entropies (SMILES Shannon and SMILES partial/ fractional Shannon) and MW as descriptors
...
### Only MW & Morgan Fingerprint as features
df_new = pd.concat([ df['Molecular Weight'], fp_morgan, df['Ligand Efficiency BEI/MW']], axis = 1)

### Both MW & Morgan Fingerprint + Shannon entropy & fractonal Shannon as features
df_new = pd.concat([ df['Molecular Weight'], df['shannon_smiles'], fp_mol, fp_morgan, df['Ligand Efficiency BEI/MW']], axis = 1)
...
plt.savefig('Ligand_Efficiency_BEI_pred_with_MW_morgan_fingerprint_only.png')
plt.show()
...
plt.ylabel("Loss")
plt.legend()
plt.savefig('loss@epochs_BEI_with_MW_morgan_fingerprint_only.png')
####------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------