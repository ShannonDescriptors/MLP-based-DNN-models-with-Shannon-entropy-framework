...
print("Shape of the pChEMBL/MW Value", df_target.shape)
...
# generating a dictionary of atom occurrence frequencies with the following atoms
...
for i in range(0,len(df['Smiles'])):  
  ...
  ### estimating the total shannon entropy based on smiles
...
### concatenating different feature combinations to feed into MLP

# fp_mol = pd.concat([ df, fp_mol], axis = 1)
...
# df_new = pd.concat([ df['Molecular Weight'], shannon_smiles, fp_mol, df['Ligand Efficiency BEI'], df['pChEMBL Value']], axis = 1)
#df_new = pd.concat([ df['Molecular Weight'], df['Ligand Efficiency BEI'], df['pChEMBL Value']], axis = 1)
df_new = pd.concat([ df['Molecular Weight'], shannon_smiles, df['Ligand Efficiency BEI'], df['pChEMBL Value']], axis = 1)
...
### Taking only the last column (pChEMBL/MW) as the target
...
# Just the 1st column (mol. wt.), 2nd col (shannon entropy) and/ or partial shannon columns and the penultimate column (Ligand efficiency BEI) as X data
...
# # FINAL MODEL taking into account numerical data from MLP and images from CNN, which combinedly send a single value (i.e. the predicted price of the house)
...
plt.savefig('pChEMBL_MW Value_with_shannon_other_descriptors_MLP_only.png')
plt.show()
...
plt.ylabel("Loss")
plt.legend()
plt.savefig('Loss_with_epochs_pChEMBL_MW Value_with_shannon_other_descriptors_MLP_only')
####------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
